...
# Overally there will be 10 options for our first element, then 9 for the 2nd...
...
for lt in [1000000]:
    ...
    nleft = len(nums)
    #lt = 2
    mlt = [] # the millionth lexographic term, it will be constructed as we go
    cvalue = 0
    ...
    while nleft > 1:
        ...
        num = 0
        while num <= nleft:
            #print repr(num) + "  " + repr(nums[num-1]) + "  " + repr(cvalue + (num)*naddperms)
        ...
        #print mlt
        nleft = len(nums)
    print repr(array2int(mlt)) + " is the " + repr(lt) + "th lexographic permutation of the set"
